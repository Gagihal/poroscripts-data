...
// @downloadURL  https://raw.githubusercontent.com/Gagihal/poroscripts-data/main/pokecreditor/pokecreditor-tcgseller-button.user.js
...
(async function () {
  ...
  (function injectStyles(){
    const css = `
      .pm-tcgseller-cell { white-space: nowrap; }
      .pm-tcgseller-wrap { display:flex; gap:4px; align-items:center; }
      .pm-tcgseller-btn {
      ...
      .pm-tcgseller-btn:active { transform: translateY(1px); }
      @media (max-width: 900px){ .pm-tcgseller-btn { padding: 1px 5px; font-size: 10px; } }
    `;
  ...
  async function enhance(){
    ...
    for (const row of rows){
      ...
      newCell.className = 'pm-tcgseller-cell';

      const holder = document.createElement('div');
      holder.className = 'pm-tcgseller-wrap';
